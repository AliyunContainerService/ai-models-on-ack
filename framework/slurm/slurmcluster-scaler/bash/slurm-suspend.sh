...
SLURM_NAMESPACE=$SLURM_NAMESPACE SLURM_CLUSTER=$SLURM_CLUSTER KUBERNETES_SERVICE_PORT=$(cat /var/k8s-info/k8s_svc_port) KUBERNETES_SERVICE_HOST=$(cat /var/k8s-info/k8s_svc_host) $SCALER suspend "$@" &>> /var/log/slurm-suspend.log
